...
export const displayCurrentItemList = (): MyThunk => (dispatch, getState) => {
    ...
    items.find(x=> x._name == "NCR");
           // z = setItems(y);
          items.slice(4,7);
         console.log(items.filter(function isBigEnough(element, index, array) { 
            return element._name == "NCR"; 
         } ))
           console.log(items.find(x=> x._name.search("p")));
            dispatch(setItems(items.filter(function isBigEnough(element, index, array) { 
                return element._name == "NCR" || element._name == "NOLO" || element._name =="POS" || element._name == "public" || element._name == "NoloOrder.txt" 
                || element._name == "Cinema.txt" || element._name == "CINEMAS" || element._name == "Banking" || element._name == "Retail" || element._name == "Finance" || element._name == "Loyalty" || element._name == "BSP" || element._name == "Engage" || element._name == "Payments" || element._name == "Aloha Takeout" || element._name == "Console" || element._name == "Venues" || element._name == "Silver"; 
             } )));
            //console.log(items);
        })
        .catch(r => dispatch(setErrorMessage(String(r))))
